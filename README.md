...
รองรับ Sheet ภาษาไทย 7 คอลัมน์ด้วย: `รหัส,หมวดหมู่,ช่วงชั้น,คำถามตัวอย่าง,คำตอบสำหรับนักเรียน,การตอบสนองเริ่มต้น,เงื่อนไขและการส่งต่อ (ผู้ดูแล)` โดยเปิดใช้ทุกแถว รูปแบบนี้ไม่มี enabled ระดับ `ทั่วไป` ใช้ action=answer; `ส่งต่อ`, `เร่งด่วน`, `ฉุกเฉิน` ใช้ action=handoff ตามข้อกำหนดส่งต่อของบอท เก็บคำตอบ หมวดหมู่ ช่วงชั้น ระดับ และเงื่อนไขผู้ดูแลไว้ใน FAQ ที่ส่งให้ Gemini ครบถ้วน การส่งต่อจริงยังไม่เชื่อมตามข้อจำกัดด้านล่าง
